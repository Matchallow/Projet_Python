...
def anomalies(variable,numero):
    ...
    if (numero in ['1','2','3','4','5','6'])==True:
        ...
        t = date_min
        delta = pd.Timedelta( days = 14 ) #Solène tu peux changer en hours = ... ou minutes =...
        anomalies = []
        points_anomalies = []
        max_jour = []
        min_jour = []
        ...
        moy_min = mediane(min_jour)
        moy_max = mediane(max_jour)
        
        # On cherche les valeurs aberrantes
        Q = capteur_numero[var].tolist()
        T = time.tolist()
        
        MAD_min = [(abs(q-moy_min)) for q in Q]
        MAD_max = [(abs(q-moy_max)) for q in Q]
        MAD = [(abs(q-mediane(Q))) for q in Q]
        
        for k in range(len(Q)):
            if Q[k] <= mediane(Q)-2*mediane(MAD) or Q[k] >= mediane(Q)+2*mediane(MAD) :
